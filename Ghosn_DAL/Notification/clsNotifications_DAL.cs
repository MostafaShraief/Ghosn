...
namespace Ghosn_DAL
{
    public class NotificationObject
    {
        ...
        public NotificationObject(int notificationID, string title, DateTime dateTime, string body)
        {
    ...
    public class clsNotifications_DAL
    {
        ...
        public static int AddNotification(NotificationObject notification)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                string query = "INSERT INTO Notifications (Title, DateAndTime, Body) VALUES (@Title, @DateAndTime, @Body); SELECT SCOPE_IDENTITY();";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
        ...
        public static bool UpdateNotification(NotificationObject notification)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                string query = "UPDATE Notifications SET Title = @Title, DateAndTime = @DateAndTime, Body = @Body WHERE NotificationID = @NotificationID";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
